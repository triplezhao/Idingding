...
import fgh.idd.chips.app.WebViewActivity;
import fgh.idd.data.bean.JiongtuAlbum;
import fgh.idd.mvp.alarm.AlarmActivity;
import fgh.idd.mvp.jiongdetail.JiongDetailActivity;
import fgh.idd.ui.GuideActivity;
...
public class PageCtrl {
    ...
    public static void startForResult(Context from, Class<?> clsAct, boolean finish, String action, Bundle data, int flags, int enterAnim, int exitAnim, int requestCode) {
    ...
    public static void start(Context from, Class<?> clsAct, boolean finish, String action, Bundle data, int flags, int enterAnim, int exitAnim) {
        startForResult(from, clsAct, finish, action, data, flags, enterAnim, exitAnim, -1);
    }

    public static void start(Context from, Class<?> clsAct, boolean finish, String action, Bundle data) {
        start(from, clsAct, finish, action, data, 0, 0, 0);
    }

    public static void start(Context from, Class<?> clsAct, boolean finish, String action, Bundle data, int enterAnim, int exitAnim) {
        start(from, clsAct, finish, action, data, 0, enterAnim, exitAnim);
    }

    public static void startForResult(Context from, Class<?> clsAct, boolean finish, String action, Bundle data, int requestCode) {
...
//        start(context, MainTabActivity.class, true, "", null);
        start(context, AlarmActivity.class, true, "", null);
    }

    public static void start2AlarmActivity(Context context) {
        start(context, AlarmActivity.class, true, "", null);
    }
